...
#### 9 Redis 主从集群中主节点异常后，从节点替换为主节点的完整流程

​		这一过程由 Redis 哨兵（Sentinel）主导，分为「故障检测→Leader 选举→故障转移」三大阶段，从节点不会立刻替换主节点，而是经过标准化流程后完成切换，全程保证集群可用性。

核心结论先明确：

| 阶段        | 核心动作                                              | 是否涉及主从切换       |
| ----------- | ----------------------------------------------------- | ---------------------- |
| 故障检测    | 主观下线（sdown）→ 客观下线（odown）                  | 否（仅确认主节点故障） |
| Leader 选举 | 哨兵集群投票选出执行故障转移的 Leader 哨兵            | 否（仅确定执行者）     |
| 故障转移    | 选新主→其他从节点复制新主→旧主复活降级为从→通知客户端 | 是（核心切换阶段）     |

**完整替换流程（分 3 大阶段，8 个关键步骤）**

​		假设集群架构：1 主（Master）+2 从（Slave1/Slave2）+3 哨兵（Sentinel1/2/3），默认配置`quorum=2`（至少 2 个哨兵确认主节点下线）。

**阶段 1：故障检测（确认主节点 “真的挂了”）**

步骤 1：单个哨兵检测到主节点异常 → 主观下线（sdown）

- 哨兵会以`down-after-milliseconds`（默认 30000ms=30 秒）为周期，向主节点发送`PING`命令；
- 若主节点在该时间内未返回有效响应（如超时、拒绝连接），该哨兵标记主节点为**主观下线（Subjectively Down）**；
- 此时仅该哨兵认为主节点故障，其他哨兵 / 从节点仍正常。

步骤 2：哨兵间互通状态 → 达到 quorum 数 → 客观下线（odown）

- 标记主观下线的哨兵会向其他哨兵发送`is-master-down-by-addr`命令，询问是否认为主节点下线；
- 当**至少 quorum 个哨兵**（本例 = 2）都确认主节点主观下线后，哨兵集群将主节点标记为**客观下线（Objectively Down）**；
- 关键：客观下线仅代表 “集群确认主节点故障”，**未开始任何切换操作**。

**阶段 2：Leader 选举（确定 “谁来执行切换”）**

步骤 3：哨兵集群发起 Leader 选举

- 所有哨兵会向其他哨兵发送`leader`竞选请求，要求成为 “故障转移执行者”；

- 遵循「Raft 协议简化版」：

  1. 每个哨兵只能投 1 票，且先到先得；
  2. 获得**超过半数哨兵**（本例 = 2，3 个哨兵需≥2 票）支持的哨兵成为 Leader；
  3. 若第一轮无胜者，等待`failover-timeout`（默认 180000ms）后重新选举。

  

步骤 4：确定 Leader 哨兵（唯一执行者）

- 最终只有 1 个哨兵成为 Leader（如 Sentinel1），负责后续所有故障转移操作；
- 其他哨兵仅作为旁观者，若 Leader 挂了则重新选举。

**阶段 3：故障转移（核心切换，从节点变主节点）**

步骤 5：Leader 哨兵筛选可用从节点

Leader 会遍历所有从节点，按以下优先级筛选 “候选新主”：

1. **排除不健康的从节点**（如断线、延迟过高）；
2. **优先选`slave-priority`值小的**（默认 100，值越小优先级越高，0 表示无竞选资格）；
3. **选复制偏移量最大的**（与旧主数据同步最完整）；
4. **选运行 ID 最小的**（兜底规则，保证唯一）；

- 本例假设 Slave1 优先级最高、偏移量最大，被选为新主。

步骤 6：将候选从节点升级为主节点

- Leader 哨兵向 Slave1 发送`SLAVEOF NO ONE`命令，Slave1 停止复制旧主，成为新主节点；
- Leader 哨兵等待 Slave1 完成角色切换（通过`INFO replication`验证）。

步骤 7：其他从节点切换到新主

- Leader 哨兵向 Slave2 发送`SLAVEOF 新主IP 新主端口`命令，Slave2 开始复制新主（Slave1）；
- 若有更多从节点，依次执行该操作，保证所有从节点同步新主数据。

步骤 8：处理旧主节点 + 通知客户端

1. 若旧主节点复活，Leader 哨兵向其发送`SLAVEOF 新主IP 新主端口`，使其降级为新主的从节点；
2. 哨兵集群通过发布订阅（`__sentinel__:hello`频道）向所有客户端推送 “新主节点地址”，客户端感知后切换连接；
3. 故障转移完成，集群恢复正常。
...
问题 1：“如果所有从节点都不可用，会发生什么？”

- 哨兵无法完成故障转移，集群进入 “无主状态”；
- 哨兵会每隔`failover-timeout`重试选举和转移，直到有可用从节点。

问题 2：“客户端如何感知新主节点？”

- 客户端需监听哨兵的`__sentinel__:hello`频道，或定期向哨兵请求 “当前主节点地址”；
- 主流 Redis 客户端（如 Jedis、Lettuce）已内置哨兵适配逻辑，可自动切换。

问题 3：“故障转移过程中，集群能读写吗？”

- 写操作：旧主挂掉后写失败，直到新主切换完成；
- 读操作：可继续从从节点读取（但可能是旧数据），新主切换后读新主。

问题4： Redis 有哨兵的主从集群中单个从节点异常后的完整处理方法

若集群部署了哨兵，从节点异常的处理会更自动化：

1. 哨兵会定期监控从节点状态，标记异常从节点为`SDOWN`（主观下线）；
2. 故障转移时，哨兵会跳过异常从节点，仅从健康从节点中选新主；
3. 异常从节点恢复后，哨兵会自动识别并将其纳入集群，无需手动配置主从；
4. 只需在哨兵配置中保证`min-slaves-to-write 1`（主节点至少有 1 个健康从节点才允许写），避免主节点 “裸奔”。



#### 10 在没有配置哨兵的 Redis 主从集群中，当主节点发生异常时，是否能自动完成主从切换？

**不能**。

Redis 的主从复制机制本身只负责数据的同步，不具备故障检测和自动切换的能力。具体来说：

1. **主从复制的核心作用**：仅实现主节点（master）的数据异步复制到从节点（slave），从节点默认是只读的，无法处理写请求。
2. 无哨兵时的故障表现：
   - 主节点宕机后，所有从节点依然会保持 “从节点” 身份，不会主动升级为主节点；
   - 整个集群无法处理写请求，读请求只能从从节点获取（但数据停留在主节点宕机前的状态）；
   - 必须由人工介入完成主从切换，步骤通常是：
     - 选出一个健康的从节点，执行 `slaveof no one` 使其成为新主节点；
     - 让其他从节点执行 `slaveof 新主节点IP 端口`，同步新主节点数据；
     - 修改应用配置，将写请求指向新主节点。
