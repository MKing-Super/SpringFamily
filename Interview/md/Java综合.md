...
| **分表类型** | **原理**                             | **适用场景**                      | **优点**                      | **缺点**                     |
| ------------ | ------------------------------------ | --------------------------------- | ----------------------------- | ---------------------------- |
| **垂直分表** | 按列拆分，将大字段或不常用字段分离   | 表列过多，包含大字段（TEXT/BLOB） | 减少I/O，提高热点数据访问速度 | 需JOIN操作，复杂查询性能下降 |
| **水平分表** | 按行拆分，数据分布到多个结构相同的表 | 单表数据量过大（>500万行）        | 分散负载，提高并发性能        | 跨表查询复杂，需要路由机制   |
| **分库分表** | 同时分库和分表                       | 超高并发和海量数据（TB级）        | 最大程度分散负载              | 架构复杂，维护成本高         |