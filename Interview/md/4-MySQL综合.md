...
1. **谓词下推**：将 `WHERE` 条件提前到 `JOIN` 前执行（减少中间数据）。
2. **索引优化**：优先使用索引满足 `WHERE`、`ORDER BY`、`GROUP BY`。
3. **子查询优化**：可能将子查询转化为 `JOIN` 操作。
...
1. **谓词下推**：`EXPLAIN` 显示分区表提前过滤
2. **索引下推**：`Using index condition`
3. **子查询优化**：`Start temporary/End temporary` 或 `MATERIALIZED`
...
**悲观锁**：默认认为并发冲突会发生，故先加锁再访问。 

- **共享锁 (S Lock / 读锁)**：允许其他事务读，但不允许写。`SELECT ... LOCK IN SHARE MODE;` 
- **排他锁 (X Lock / 写锁)**：不允许其他事务读和写。`SELECT ... FOR UPDATE;`
