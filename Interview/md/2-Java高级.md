...
- 方法的“代码”（字节码指令）存储在方法区（元空间）中。
- 方法的“执行过程”发生在虚拟机栈的栈帧中。
- 实际“干活”的、真正执行每条指令的是执行引擎（具体是指令解释器或JIT编译后的本地代码）。

​            **方法本身（代码）存储在方法区。方法的每一次被调用，其运行时的状态和数据存储在虚拟机栈的栈帧中。而“执行”这个动作，是由执行引擎通过操作栈帧中的数据，并解释或编译方法区的字节码来完成的。**
...
这是最基础和原始的算法，后续算法都基于它的改进。

**原理分为两步：**

**a 标记（Mark）**： 

- GC 从一组 **“GC Roots”**（如活动线程栈的引用、静态变量等）开始，遍历所有能被引用链访问到的对象。 
- 将这些 **“存活”** 的对象在内存中打上一个标记（例如，在对象头中设置一个标记位）。
- 遍历完成后，所有**未被标记**的对象就是**垃圾对象**。

**b 清除（Sweep）**： 

- 再次遍历整个堆内存。 
- 对于**未被标记**的对象，直接将其占用的内存空间**回收为空闲状态**（通常记录在一个“空闲列表”里，供后续分配使用）。
- 对于被标记的对象，则**清除其标记位**，为下一次GC做准备。

**优点：**

- 实现简单。

- 不需要移动存活对象，适用于存活对象很多、回收对象很少的大对象或老年代（在某些场景下）。

**缺点：**

- **内存碎片化严重**：回收后会产生大量不连续的内存碎片。当需要分配一个大对象时，即使总空闲内存足够，也可能因为找不到一个连续的大空间而触发另一次GC。
- **效率问题**：需要遍历两次堆内存（标记一次，清除一次），在堆很大的情况下效率较低。



**2. 复制（Copying）算法**

为了解决标记-清除的碎片问题而生，但它需要付出额外的空间代价。

**原理：**

**a 划分空间**：

​		将可用的堆内存**等分为两块**，每次只使用其中一块，比如 **From 空间**。

**b 复制与回收**： 

- 当 **From 空间** 快满时，触发GC。 
- 从 GC Roots 开始遍历，将 **From 空间** 中所有**存活**的对象，**整体复制到另一块空闲的 To 空间**。 
- 在复制过程中，对象会被紧凑地排列在 **To 空间** 的一端，完全**没有碎片**。
- 复制完成后，**一次性清空整个 From 空间**。

**c 角色互换**：

​		之后，将 **To 空间** 作为新的活动空间（From），原来的 **From 空间** 变为新的空闲空间（To）。

**优点：**

- **无内存碎片**：每次回收后，存活对象都被紧凑地排列在一起。
- **高效**：只需要遍历一次存活对象（一边标记，一边复制）。清除过程是整块内存的清空，极快。

- 实现简单，运行高效。

**缺点：**

- **内存利用率低**：在任何时刻，都有一半的内存是完全闲置不用的。这是最大的代价。

- 当**存活对象数量很多**时，复制的开销会变得非常大。

**适用场景**：因为其“存活对象越少，效率越高”的特性，它**非常适合对象“朝生夕死”的年轻代（Young Generation）**。在 HotSpot JVM 的年轻代（分为 Eden 区和两个 Survivor 区）中，使用的就是改进的复制算法。



### 3. 标记-整理（Mark-Compact）算法

综合了前两者的优点，也被称为“标记-清除-整理”算法。它解决了碎片问题，又不像复制算法那样浪费空间。

**原理分为三步：**

**a 标记（Mark）**：

​		与“标记-清除”算法完全一样，从 GC Roots 开始标记所有存活对象。

**b 整理（Compact）**： 

- 将所有**存活的对象**，**向内存空间的一端进行移动**。
- 移动完成后，所有存活对象都被**紧凑地排列**在内存的一端。

**c 清理边界（Sweep）**：

​		最后，直接**清理掉边界以外的所有内存**（即，存活对象边界之后的所有空间全部被视为空闲空间）。
...
- **效率相对较低**：需要移动所有存活对象，并且更新所有指向这些对象的引用地址，**“整理”阶段的开销是三种算法中最大的**。其停顿时间通常长于复制和标记-清除。

**适用场景**：因为需要移动对象并更新引用，所以通常用于**存活对象多、更新不频繁的老年代（Old Generation）**，例如 Serial Old 和 Parallel Old 收集器使用的就是标记-整理算法。
...
public class DeadlockDemo {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("主线程结束 - 这行代码很可能不会执行，因为死锁了！");
    }
}
```
...
​		锁 `lock1`和 `lock2`一次只能被一个线程持有。

**占有且等待**： 

- 线程1 持有 `lock1`，同时等待获取 `lock2`。
- 线程2 持有 `lock2`，同时等待获取 `lock1`。

**不可剥夺**：

​		JVM 的 `synchronized`锁不支持强制抢占，线程不会主动释放已持有的锁。

**循环等待**：

​		线程1 等待线程2 释放 `lock2`，线程2 等待线程1 释放 `lock1`，形成一个循环等待链。

> join() 方法的作用：
>
> ​		join()方法的作用是让主线程等待子线程执行完毕，它本身不会影响子线程之间的并发执行和锁竞争。
>
> join()只影响主线程：
>
> - 不加 join()：主线程启动两个子线程后立即继续执行，打印"主线程结束"（但实际上由于死锁，子线程永远不会结束，主线程实际上是在"后台"等待子线程结束）。
> - 加 join()：主线程明确地等待两个子线程结束，然后才打印"主线程结束"。
>
> **死锁是由线程间的锁竞争顺序造成的，与 `join()`方法无关。`join()`只是影响了主线程是否显式等待子线程结束。**
