...
1 . **请求分发**：`DispatcherServlet` （前端控制器）接收所有请求

2 .**处理器映射**：通过 `HandlerMapping` （处理器映射器）找到处理该请求的 Handler（Controller 方法）

3 .**处理器适配**：通过 `HandlerAdapter`（处理器适配器） 找到能执行该 Handler 的适配器

4 .**拦截器预处理**：按顺序执行拦截器的 `preHandle()`方法

5 .**调用 Controller**：`HandlerAdapter` （处理器适配器）调用实际的 Controller 方法

6 .**参数绑定**：

- 从请求中提取参数 
- 执行数据绑定和验证（@Valid） 
- 处理 @RequestParam、@PathVariable、@RequestBody 等注解

7 .**业务逻辑**：执行 Controller 中的业务代码

8 .**返回结果**：方法返回 ModelAndView 或具体对象

9 .**视图解析**：ViewResolver 根据视图名解析出具体的 View 对象

10 .**执行 postHandle**：执行拦截器的 `postHandle()`方法

11 .**视图解析**：`ViewResolver` （视图解析器）解析视图名称到具体视图

12 .**渲染响应**：视图使用模型数据渲染最终响应
...
@ResponseBody			// 将方法返回值直接写入 HTTP 响应体（Response Body），而不是跳转到视图页面。
```
...
1 .**JDK 动态代理**：基于接口代理（实现相同接口）

2 .**CGLIB 代理**：基于继承代理（生成子类）
...
1. **解耦**：组件之间的依赖关系减少，代码更加灵活、模块化。符合“面向接口编程，而不是面向实现”的原则。
2. **可测试性**：易于模拟依赖进行单元测试。
3. **可维护性**：代码更清晰，更容易理解和扩展。要更换实现，通常只需修改配置即可，无需改动业务代码。
4. **通用性**：容器提供了许多企业级服务的统一管理方式（如事务、AOP），让开发者能更专注于业务逻辑。
...
1 .你告诉 Spring：“**帮我把这些操作打包成一个整体**”（用 `@Transactional` 注解标记一个方法）。

2 .Spring 会在这个方法开始前**自动开启事务**。

3 .然后执行你方法里的所有数据库操作。

4 .最后： 

- 如果方法**顺利执行完**，Spring **自动提交事务**，所有操作正式生效。 
- •如果方法执行中**抛出了异常**，Spring **自动回滚事务**，所有操作就像没发生过一样。
...
1. **样板代码**：每次都要写重复的 `try-catch-finally` 代码块。
2. **污染业务代码**：事务管理（开启、提交、回滚）的代码和核心业务代码混杂在一起，难以维护。
3. **资源泄露风险**：容易忘记关闭连接等资源。
4. **难以切换**：如果从 JDBC 换成 Hibernate，事务 API 完全不同，需要重写所有事务代码。

**Spring 事务的优势：**

1. **声明式管理**：通过简单的注解（如 `@Transactional`）即可管理事务，**业务代码变得非常纯净**，只关注业务逻辑。
2. **统一抽象**：提供了一套通用的事务管理接口，无论底层用 JDBC、Hibernate、JPA 还是其他技术，上层用法完全一致。**更换持久层框架时，事务代码无需修改**。
3. **自动资源管理**：自动处理连接的获取、释放和回滚，避免了资源泄露。
4. **丰富的功能**：支持复杂的事务特性，如**传播行为**、**隔离级别**等。
...
| **特性**         | **说明**                                                     |
| ---------------- | ------------------------------------------------------------ |
| **作用域**       | **`SqlSession` 级别**（一般一个事务或一次请求范围内）        |
| **默认状态**     | **默认开启**，无需配置                                       |
...
**问题 1：分布式环境下，MyBatis 的一级缓存会带来数据不一致**。

```
MyBatis 的一级缓存默认开启，作用域是 SqlSession（一般一个事务或一次请求范围内），并且在同一个 SqlSession 中执行相同的查询会直接返回缓存的对象，不会再次查询数据库。

在你的例子中：
服务器 A​ 查询 id=1的用户，一级缓存中存了这个数据。
服务器 B​ 执行了更新操作（例如更新 id=1的用户信息）。
服务器 A​ 再次查询 id=1的用户时，由于一级缓存还在，它依然返回旧数据，不会去数据库重新查询，造成脏读。

这是典型的多应用实例 + 本地缓存导致的数据不一致问题，因为 MyBatis 一级缓存无法感知其他进程对数据库的更改。
```

解决方案：

**1 关闭一级缓存**（不推荐，影响性能且不符合设计初衷）

​		可以在 `<select>`标签中设置 `flushCache="true"`，或全局调整，但会频繁清理缓存，失去缓存优势。

**2 合理控制一级缓存的生命周期** 

​		在涉及可能被其他服务修改的查询时，在查询前**手动清空缓存**：

**3 使用二级缓存并配置分布式缓存**（如 Redis） 

​		MyBatis 二级缓存可以配置成集中式缓存（例如通过 Redis 实现），这样所有服务器共享同一份缓存数据，并且更新时可清除公共缓存。

**4 业务层面避免长会话** 

​		在 Web 应用中，通常一个请求结束后就关闭 SqlSession，这样一级缓存不会长时间持有旧数据。但如果请求内多次查询相同数据，仍可能遇到此问题。



**问题 2：每次 SqlSession 后缓存会被清除吗？**

​		是的。SqlSession 关闭时，其内部的一级缓存（通常是一个 `PerpetualCache`对象）会被丢弃（GC 回收），下次新建的 SqlSession 会使用新的空缓存。



**问题 3：SqlSession 与 JDBC 连接的关系**

- JDBC 连接是物理长连接，由连接池维护。

- 一个 **SqlSession** 在需要执行 SQL 时，会从连接池借一个 JDBC 连接，用完后归还（如果未开启事务）。
- 一个 SqlSession 在其生命周期内**可能使用不同的 JDBC 连接**（比如跨多次请求时），但在一次执行中通常使用同一个连接。

- 反过来，**一个 JDBC 连接在同一时间只能被一个 SqlSession 使用**（避免并发混乱）。

**关键点**：SqlSession 是 MyBatis 的会话对象，JDBC 连接是底层的物理资源，它们是多对多的关系，但受连接池管理。



**问题 4：SqlSession 的最大数量受什么限制？**

- **不受 JDBC 连接数直接限制**，因为 SqlSession 可以创建很多，只是在需要执行 SQL 时才去申请连接。
- 但如果**所有 SqlSession 同时执行 SQL**，那么并发数受限于**连接池的最大连接数**（例如 `maxPoolSize=20`，则最多 20 个 SqlSession 能同时拿到连接执行，其他会等待或报错）。
- SqlSession 本身是 Java 对象，创建过多会消耗内存，但一般不会先于连接池成为瓶颈。
...
1. **避免 SQL 拼接**：不再需要 Java 字符串拼接
2. **防止 SQL 注入**：自动使用预编译语句
3. **提高可读性**：SQL 与 Java 代码分离
4. **条件灵活组合**：根据不同参数生成不同 SQL
5. **减少重复代码**：通过 SQL 片段重用
...
1. **自动发现机制**（配置来源：spring.factories）
2. **条件过滤系统**（各种@Conditional注解，Spring Boot 使用一系列条件注解进行智能判断）
3. **属性绑定机制**（使用@ConfigurationProperties 将`application.properties/yml`中的配置绑定到Bean）
...
1. **动态检测**：扫描类路径（Classpath）中的依赖库
2. **条件判断**：根据存在的类/配置决定是否创建特定Bean
3. **智能装配**：自动配置组件及其依赖关系
...
1. **启动扫描**：`@EnableAutoConfiguration` 触发自动配置加载
2. **读取配置**：加载所有 `META-INF/spring.factories` 中声明的配置类
3. **条件过滤**：通过条件注解筛选出符合条件的配置类
4. **Bean装配**：按顺序实例化并注册符合条件的Bean
...
​		Spring Boot 是 Spring 的一套快速配置脚手架，可以基于Spring Boot 快速开发单个微服务，Spring Cloud是一个基于Spring Boot实现的开发工具；Spring Boot专注于快速、方便集成的单个微服务个体，Spring Cloud关注全局的服务治理框架； Spring Boot使用了约定优于配置的理念，很多集成方案已经帮你选择好了，能不配置就不配置，Spring Cloud很大的一部分是基于Spring Boot来实现，必须基于Spring Boot开发。
