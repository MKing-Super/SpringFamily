...
| 层级    | 全称           | 核心定位       | 粒度            | 典型表模型                         | 核心任务                                            |
| ------- | -------------- | -------------- | --------------- | ---------------------------------- | --------------------------------------------------- |
| **ODS** | 操作数据存储层 | 数据接入与溯源 | 原始业务记录    | Duplicate Key / Primary Key        | 原封不动承接 Kafka / 业务库数据，支持 Upsert/Append |
| **DWD** | 明细宽表层     | 标准化明细基石 | 业务事件粒度    | Primary Key（去重）/ Duplicate Key | 清洗、去重、维度退化、标准化，支持实时更新          |
| **DIM** | 公共维度层     | 统一维度字典   | 实体全量        | Primary Key                        | 维表统一维护，支持实时更新 / 拉链表，加速 Join      |
| **DWS** | 汇总服务层     | 公共指标预聚合 | 主题域聚合粒度  | Aggregate Key / Primary Key        | 按主题轻度 / 中度聚合，预计算公共指标，减少重复计算 |
| **ADS** | 应用数据层     | 业务指标落地   | 报表 / 接口粒度 | Primary Key / Aggregate Key        | 面向报表、大屏、API 的高度聚合，支持即席查询        |

### 2 各层详细设计

#### 1 ODS 层（原始数据接入）

- **接入方式**：Flink + Stream Load/Upsert Stream Load 从 Kafka 消费，或批量导入业务库快照。
- 表模型选择：
  - 日志类 / 不可变数据：**Duplicate Key**（高写入吞吐）。
  - 需 Upsert 的业务数据：**Primary Key**（支持主键更新）。

#### 2 DWD 层（明细宽表核心）

- **核心操作**：清洗（去重、补空、异常值处理）、维度退化（关联 DIM 层补充维度字段）、标准化（字段命名、编码统一）。
- **表模型选择**：**Primary Key**（确保业务唯一性，支持实时 Upsert）；日志类可保留 Duplicate Key。

#### 3 DIM 层（公共维度统一）

- 维度类型：
  - 静态维度：商品、用户、商家（全量保留，实时更新）。
  - 缓慢变化维度（SCD2）：用拉链表记录历史版本（如用户等级变更）。
  - 公共维度：日期、地区、渠道（固定字典）。
- **表模型选择**：**Primary Key**（支持实时 Upsert，加速 DWD/DWS 层 Join）。

#### 4 DWS 层（预聚合提速）

- **核心操作**：按主题域（订单、用户、流量）做轻度 / 中度聚合，预计算公共指标（如日订单数、月支付金额、用户累计消费）。
- 表模型选择：
  - 明确聚合指标：**Aggregate Key**（自动聚合，查询性能最优）。
  - 需保留明细的聚合：**Primary Key**（如用户每日行为宽表）。
- **物化视图**：用于复杂维度聚合，自动刷新并支持查询路由，降低开发成本。

#### 5 ADS 层（业务应用落地）

- **核心操作**：面向具体业务需求（报表、大屏、API）的高度聚合，如 “财务月收支报表”“市场渠道效果表”。
- **表模型选择**：**Primary Key**（确保指标唯一性）或 **Aggregate Key**（纯聚合报表）。
